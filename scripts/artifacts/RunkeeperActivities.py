...
def get_run_activities(files_found, report_folder, seeker, wrap_text):
    ...
    if usageentries > 0:
        ...
        for row in all_rows:
            ...
            if map:
                map_link = (
                    f'<a href="#{id}" class="btn btn-light btn-sm runkeeper-activities-map-link">'
                    'Show Map</a>'
                )
                if use_network:
                    data_list.append(
                        (
                            ...
                            row[9],
                            row[10],
                            '<a href=Runkeeper/' + str(row[0]) + '.kml class="badge badge-light" target="_blank">' + str(row[0]) + '.kml</a>',
                            '<a href=Runkeeper/' + str(row[0]) + '.xlsx class="badge badge-light" target="_blank">' + str(row[0]) + '.xlsx</a>',
                            map_link,
                        )
                    )
                else:
                    data_list.append((row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], '<a href=Runkeeper/'+str(row[0])+'.kml class="badge badge-light" target="_blank">'+str(row[0])+'.kml</a>', 'N/A', map_link))
            else:
        ...
        report.script_code += """
<script>
$(document).on('click', 'a.runkeeper-activities-map-link', function(event) {
